window.addEventListener('load', () => {
    ...
    change_password.addEventListener("click", function() {
        ...
        form.innerHTML = `
            ...
            pattern="(?=.*[A-Z])(?=.*[!@#$%^&*])(?=.*[a-z]).{8,}" 
            title="Password must contain at least 8 characters, one uppercase letter, and one special character" 
            required><br><br>
            <span id="comm"></span><br><br>
            <button type="submit" id="submit_password" class="submit-password"> Login </button>
    ...
    // Dodanie obrazka 
    function addObjectImgForProfile(image){
    ...
    // Dodanie opisu
    function addObjectDescForProfile(name, description){
    ...
    // Dodanie ocen
    function addObjectRatingForProfile(object_id){
        //Oceny
        ...
        return { ratingbox, utrashcan }
    }

    // Dodanie komentarzy
    function addObjectCommsForProfile(object_id, userID, userNAME, utrashcan, activitybox){
    ...
    // Dodanie ocen użytkownika
    function addObjectUserRatingForProfile(object_id, userID, utrashcan){
    ...
    function LoadProfileActivity(){
        ...
        const divbox = document.querySelector('.Activity-box');
        divbox.innerHTML = '';
        fetch(`api/profile/object/activity/${userID}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        .then(data => {
            data.forEach(e =>{
                ...
                // Obrazek obiektu
                const objimg = addObjectImgForProfile(image)
                activitybox.appendChild(objimg);
                // Nazwa i opis obiektu
                const descriptionbox = addObjectDescForProfile(name, description)
                activitybox.appendChild(descriptionbox);
                // Średnia ocen i ocena użytkownika
                const { ratingbox, utrashcan } = addObjectRatingForProfile(object_id)
                addObjectUserRatingForProfile(object_id, userID, utrashcan)
                activitybox.appendChild(ratingbox);
                // Komentarze użytkownika
                const filledActivitybox = addObjectCommsForProfile(object_id, userID, userNAME, utrashcan, activitybox)
                divbox.appendChild(filledActivitybox);
            });
        })
        .catch(error => console.error(error));
    };
